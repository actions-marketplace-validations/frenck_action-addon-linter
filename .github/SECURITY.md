...
GitHub Action. Users are strongly encouraged to keep their workflows up
to date.
...
[**Report a vulnerability**](https://github.com/frenck/action-app-linter/security/advisories/new)
...
- Affected version(s) of the action.
...
- Issues in the Home Assistant apps or repositories that use this action;
  please report those to their respective projects.
- Issues resulting from malformed or hostile app configuration files in
  the repository the action is explicitly pointed at. This action assumes the
  repository content it runs against is trusted.

## Scope

This security policy covers the `frenck/action-app-linter` GitHub Action
